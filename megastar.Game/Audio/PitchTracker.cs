...
public class PitchTracker
{
    ...
    public void ProcessBuffer(float[] inBuffer, int sampleCount = 0)
    {
        ...
        while (samplesProcessed < srcLength)
        {
            ...
            while (m_circularBufferLo.Read(m_pitchBufLo, CurrentPitchSamplePosition,
                       m_pitchBufSize + m_detectOverlapSamples))
            {
                float detectedPitch = 0;

                m_circularBufferHi.Read(m_pitchBufHi, CurrentPitchSamplePosition,
                    m_pitchBufSize + m_detectOverlapSamples);
    ...
    void Setup()
    {
        ...
        m_dsp = new PitchProcessor(m_sampleRate, MinimumDetectedFrequency, MaximumDetectedFrequency,
        ...
        m_pitchBufSize = (int)((1.0f / MinimumDetectedFrequency * 2.0f + (kAvgCount - 1) * kAvgOffset) * m_sampleRate) +
                         16;
